...
class LaporanController extends Controller
{
    ...
    public function downloadlap(Request $request)
    {
        ...
        $lap = DB::select('SELECT * FROM `rekammedis` 
        LEFT JOIN `polikliniks` ON rekammedis.id_poli = polikliniks.id 
        LEFT JOIN `pasiens` ON rekammedis.id_pasien = pasiens.id
        LEFT JOIN `dokters` ON rekammedis.id_dokter = dokters.id
        LEFT JOIN `perawats` ON rekammedis.id_perawat = perawats.id
        LEFT JOIN `obats` ON rekammedis.id_obat = obats.id
